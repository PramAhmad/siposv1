...
const getSiswa = async ()=>{
  ...
  loading.value = false
   const {data,error} = await supabase
    .from('siswa')
    .select("id,peserta_didik_id,nama,jenis_kelamin,nisn,rombel_saat_ini,rombel,created_at")
        ...
        <table>
            ...
            <tbody >
                ...
                <tr v-for="data,i in results" :key="i">
                    ...
                    <td data-label="Nisn">{{ data.nisn }}</td>
                    <td data-label="romboo">{{ data.rombel }}</td>
                    <td data-label="rombel saat ini">{{ data.rombel_saat_ini }}</td>                
