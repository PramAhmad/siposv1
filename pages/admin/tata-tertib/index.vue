...
const deleteTataTertib = async (id)=>{
...
const filterTataTertib = async (e)=>{

  const { data, error } = await supabase
    .from('tata_tertib')
    .select("id,jenis(id,nama),nama,konsekuensi,poin")
    .order('created_at', { ascending: false })
    // search by nama
        ...
        <table>
            ...
            <tbody >
                ...
                <tr v-for="data,i in results" :key="i">
                    ...
                    <td data-label="jenis">{{ data.jenis.nama }}</td>
                    <td data-label="konsekuensi">{{ data.konsekuensi }}</td> 
                    <td data-label="poin">{{ data.poin }}</td>               


                    <td class="before:hidden lg:w-1 whitespace-nowrap ">
                      <div class="flex gap-3">

                        <NuxtLink class="py-1 px-1.5 rounded-md border "  :to="`/admin/tata-tertib/`+data.id" >detail</NuxtLink>
                      <button  @click="deleteTataTertib(data.id)" class="py-1 px-2 rounded-md border border-rose-600 text-rose-600">delete</button>
                      </div>
                    </td>
                </tr>
            </tbody>
