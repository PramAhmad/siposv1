...
class SSHGenerateCommand extends base_1.SSHBaseCommand {
    async getMetadata() {
        return {
            ...
            options: [
                {
                    name: 'type',
                    summary: `The type of key to generate: ${SSH_KEY_TYPES.map(v => (0, color_1.input)(v)).join(', ')}`,
                    default: 'rsa',
                    aliases: ['t'],
    ...
    async preRun(inputs, options) {
        ...
        (0, cli_framework_1.validate)(String(options['type']), 'type', [(0, cli_framework_1.contains)(SSH_KEY_TYPES, { caseSensitive: false })]);
    }
    async run(inputs, options) {
        const { getGeneratedPrivateKeyPath } = await Promise.resolve().then(() => require('../../lib/ssh'));
        const { bits, annotation } = options;
        const keyPath = inputs[0] ? (0, utils_terminal_1.expandPath)(String(inputs[0])) : await getGeneratedPrivateKeyPath(this.env.config.get('user.id'));
        const keyPathDir = path.dirname(keyPath);
        const pubkeyPath = `${keyPath}.pub`;
        if (!(await (0, utils_fs_1.pathExists)(keyPathDir))) {
            await (0, utils_fs_1.mkdirp)(keyPathDir, 0o700);
            this.env.log.msg(`Created ${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(keyPathDir))} directory for you.`);
        }
        if (await (0, utils_fs_1.pathExists)(keyPath)) {
            const confirm = await this.env.prompt({
                type: 'confirm',
                name: 'confirm',
                message: `Key ${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(keyPath))} exists. Overwrite?`,
            });
            if (confirm) {
                await (0, utils_fs_1.unlink)(keyPath);
            }
            else {
                this.env.log.msg(`Not overwriting ${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(keyPath))}.`);
                return;
            }
        }
        this.env.log.info('Enter a passphrase for your private key.\n' +
            `You will be prompted to provide a ${(0, color_1.strong)('passphrase')}, which is used to protect your private key should you lose it. (If someone has your private key, they can impersonate you!) Passphrases are recommended, but not required.\n`);
        ...
        this.env.log.nl();
        this.env.log.rawmsg(`Private Key (${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(keyPath))}): Keep this safe!\n` +
