...
class RunCommand extends base_1.CapacitorCommand {
    ...
    async getMetadata() {
        ...
        let options = [
            {
                ...
                hint: (0, color_1.weak)('[capacitor]'),
            },
            {
                name: 'target',
                summary: `Deploy to a specific device by its ID (use ${(0, color_1.input)('--list')} to see all)`,
                type: String,
                groups: ['capacitor', 'native-run'],
                hint: (0, color_1.weak)('[capacitor]'),
            },
            {
                name: 'open',
                summary: `Open native IDE instead of using ${(0, color_1.input)('capacitor run')}`,
                type: Boolean,
            },
            // Build Options
            {
        ...
        return {
            ...
            description: `
${(0, color_1.input)('ionic capacitor run')} will do the following:
- Perform ${(0, color_1.input)('ionic build')} (or run the dev server from ${(0, color_1.input)('ionic serve')} with the ${(0, color_1.input)('--livereload')} option)
- Run ${(0, color_1.input)('capacitor run')} (or open IDE for your native project with the ${(0, color_1.input)('--open')} option)
...
If you have multiple devices and emulators, you can target a specific one by ID with the ${(0, color_1.input)('--target')} option. You can list targets with ${(0, color_1.input)('--list')}.
...
For Android and iOS, you can setup Remote Debugging on your device with browser development tools using these docs[^remote-debugging-docs].
      `,
            ...
            inputs: [
                {
                    name: 'platform',
                    summary: `The platform to run (e.g. ${PLATFORMS.map(v => (0, color_1.input)(v)).join(', ')})`,
    ...
    async preRun(inputs, options, runinfo) {
        ...
        if (await this.isOldCapacitor()) {
            this.env.log.warn(`Support for Capacitor 1 and 2 is deprecated.\n` +
                `Please update to the latest Capacitor. Visit the docs${(0, color_1.ancillary)('[1]')} for upgrade guides.\n\n` +
                `${(0, color_1.ancillary)('[1]')}: ${(0, color_1.strong)('https://capacitorjs.com/docs/')}\n`);
            if (options['list']) {
                throw new errors_1.FatalException(`The ${(0, color_1.input)('--list')} option is not supported with your Capacitor version.`);
            }
            if (options['target']) {
                throw new errors_1.FatalException(`The ${(0, color_1.input)('--target')} option is not supported with your Capacitor version.`);
            }
        }
        if (!inputs[0]) {
        ...
        if (!(await this.isOldCapacitor())) {
            ...
            if (options['list']) {
                if (!inputs[0]) {
                    throw new errors_1.FatalException(`The ${(0, color_1.input)('platform')} argument is required for the ${(0, color_1.input)('--list')} option.`);
                }
                if (targets.length > 0) {
                    const rows = targets.map(t => [t.name, t.api, t.id]);
                    this.env.log.msg((0, utils_terminal_1.columnar)(rows, { ...constants_1.COLUMNAR_OPTIONS, headers: ['Name', 'API', 'Target ID'] }));
                }
                else {
            ...
            if (!options['open']) {
                ...
                if (typeof target === 'string') {
                    if (!targets.map(t => t.id).find(t => t === target)) {
                        throw new errors_1.FatalException(`${(0, color_1.input)(target)} is not a valid Target ID.\n` +
                            `Use the ${(0, color_1.input)('--list')} option to list all targets.\n`);
                    }
                }
                else {
                    if (targets.length > 0) {
                    ...
                    if (!inputs[0]) {
                        throw new errors_1.FatalException(`The ${(0, color_1.input)('platform')} argument is required.`);
                    }
                    if (!options['target']) {
                        throw new errors_1.FatalException(`The ${(0, color_1.input)('--target')} option is required.`);
                    }
                }
            }
        }
    ...
    getContinueMessage(platform) {
        ...
        return ('Ready for use in your Native IDE!\n' +
            `To continue, run your project on a device or ${(0, utils_1.getVirtualDeviceNameForPlatform)(platform)} using ${(0, utils_1.getNativeIDEForPlatform)(platform)}!`);
    }
