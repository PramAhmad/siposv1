...
class IntegrationsEnableCommand extends command_1.Command {
    async getMetadata() {
        return {
            ...
            description: `
Integrations, such as Cordova, can be enabled with this command. If the integration has never been added to the project, ${(0, color_1.input)('ionic integrations enable')} will download and add the integration.

Integrations can be re-added with the ${(0, color_1.input)('--add')} option.
      `,
            inputs: [
                {
                    name: 'name',
                    summary: `The integration to enable (e.g. ${integrations_1.INTEGRATION_NAMES.map(i => (0, color_1.input)(i)).join(', ')})`,
                    validators: [cli_framework_1.validators.required, (0, cli_framework_1.contains)(integrations_1.INTEGRATION_NAMES, {})],
                },
            ],
            options: [
                {
    ...
    async run(inputs, options) {
        ...
        if (!this.project) {
            throw new errors_1.FatalException(`Cannot run ${(0, color_1.input)('ionic integrations enable')} outside a project directory.`);
        }
        ...
        if (!(0, guards_1.isIntegrationName)(name)) {
            throw new errors_1.FatalException(`Don't know about ${(0, color_1.input)(name)} integration!`);
        }
        ...
        try {
            if (!integration.isAdded() || add) {
                ...
                this.env.log.ok(`Integration ${(0, color_1.input)(integration.name)} added!`);
            }
            else {
                ...
                if (wasEnabled) {
                    this.env.log.info(`Integration ${(0, color_1.input)(integration.name)} already enabled.`);
                }
                else {
                    this.env.log.ok(`Integration ${(0, color_1.input)(integration.name)} enabled!`);
                }
            }
        }
        catch (e) {
