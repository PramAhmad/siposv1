...
const NATIVE_RUN_OPTIONS = [
    {
        name: 'native-run',
        summary: `Do not use ${(0, color_1.input)('native-run')} to run the app; use Cordova instead`,
        type: Boolean,
        default: true,
        groups: ['native-run'],
        hint: (0, color_1.weak)('[native-run]'),
    },
    {
        name: 'connect',
        ...
        type: Boolean,
        groups: ['native-run'],
        hint: (0, color_1.weak)('[native-run] (--livereload)'),
    },
    {
        name: 'json',
        summary: `Output targets in JSON`,
        type: Boolean,
        groups: ["advanced" /* MetadataGroup.ADVANCED */, 'native-run'],
        hint: (0, color_1.weak)('[native-run] (--list)'),
    },
];
class RunCommand extends base_1.CordovaCommand {
    async getMetadata() {
        ...
        let options = [
            ...
            // Serve Options
            ...serve_1.COMMON_SERVE_COMMAND_OPTIONS.filter(o => !['livereload'].includes(o.name)).map(o => ({ ...o, hint: (0, color_1.weak)('(--livereload)') })),
            {
                name: 'livereload',
        ...
        if (serveRunner) {
            ...
            const runnerOpts = (libmetadata.options || [])
                ...
                .map(o => ({ ...o, hint: `${o.hint ? `${o.hint} ` : ''}${(0, color_1.weak)('(--livereload)')}` }));
        ...
        return {
            ...
            description: `
Build your app and deploy it to devices and emulators using this command. Optionally specify the ${(0, color_1.input)('--livereload')} option to use the dev server from ${(0, color_1.input)('ionic serve')} for livereload functionality.

This command will first use ${(0, color_1.input)('ionic build')} to build web assets (or ${(0, color_1.input)('ionic serve')} with the ${(0, color_1.input)('--livereload')} option). Then, ${(0, color_1.input)('cordova build')} is used to compile and prepare your app. Finally, the ${(0, color_1.input)('native-run')} utility[^native-run-repo] is used to run your app on a device. To use Cordova for this process instead, use the ${(0, color_1.input)('--no-native-run')} option.

If you have multiple devices and emulators, you can target a specific one with the ${(0, color_1.input)('--target')} option. You can list targets with ${(0, color_1.input)('--list')}.
...
When using ${(0, color_1.input)('--livereload')} with hardware devices, remember that livereload needs an active connection between device and computer. In some scenarios, you may need to host the dev server on an external address using the ${(0, color_1.input)('--external')} option. See these docs[^livereload-docs] for more information.

Just like with ${(0, color_1.input)('ionic cordova build')}, you can pass additional options to the Cordova CLI using the ${(0, color_1.input)('--')} separator. To pass additional options to the dev server, consider using ${(0, color_1.input)('ionic serve')} separately and using the ${(0, color_1.input)('--livereload-url')} option.
      `,
            footnotes,
            exampleCommands,
            inputs: [
                {
                    name: 'platform',
                    summary: `The platform to run (e.g. ${['android', 'ios'].map(v => (0, color_1.input)(v)).join(', ')})`,
    ...
    async preRun(inputs, options, runinfo) {
        ...
        if (!options['build'] && options['livereload']) {
            this.env.log.warn(`No livereload with ${(0, color_1.input)('--no-build')}.`);
        ...
        if (options['list']) {
            if (options['native-run']) {
                const args = (0, native_run_1.createNativeRunListArgs)(inputs, options);
                await this.runNativeRun(args);
            }
            else {
                const args = (0, utils_1.filterArgumentsForCordova)(metadata, options);
        ...
        if (!inputs[0]) {
            const p = await this.env.prompt({
                type: 'input',
                name: 'platform',
                message: `What platform would you like to run (${['android', 'ios'].map(v => (0, color_1.input)(v)).join(', ')}):`,
            });
            inputs[0] = p.trim();
        }
        const [platform] = inputs;
        if (platform && options['native-run'] && !native_run_1.SUPPORTED_PLATFORMS.includes(platform)) {
            this.env.log.warn(`${(0, color_1.input)(platform)} is not supported by ${(0, color_1.input)('native-run')}. Using Cordova to run the app.`);
        ...
        if (options['native-run'] && !options['device'] && !options['emulator'] && platform) {
            const platformTargets = await (0, native_run_1.getNativeTargets)(this.env, platform);
            const { devices } = platformTargets;
            debug(`Native platform devices: %O`, devices);
            if (devices.length > 0) {
                this.env.log.info(`Hardware device(s) found for ${(0, color_1.input)(platform)}. Using ${(0, color_1.input)('--device')}.`);
                options['device'] = true;
            }
        }
    ...
    async runServeDeploy(inputs, options) {
        ...
        const metadata = await this.getMetadata();
        if (!this.project) {
            throw new errors_1.FatalException(`Cannot run ${(0, color_1.input)(`ionic cordova ${metadata.name}`)} outside a project directory.`);
        }
        const runner = await this.project.requireServeRunner();
        const runnerOpts = runner.createOptionsFromCommandLine(inputs, (0, utils_1.generateOptionsForCordovaBuild)(metadata, inputs, options));
        /**
        ...
        if (!serverUrl) {
            ...
            if (details.externallyAccessible === false && !options['native-run']) {
                ...
                this.env.log.warn(`Your device or emulator may not be able to access ${(0, color_1.strong)(details.externalAddress)}.${extra}\n\n`);
            }
        ...
        await conf.save();
        const cordovalogws = (0, logger_1.createPrefixedWriteStream)(this.env.log, (0, color_1.weak)(`[cordova]`));
        ...
        if (options['native-run']) {
            const [platform] = inputs;
            await this.runCordova((0, utils_1.filterArgumentsForCordova)({ ...metadata, name: 'build' }, options), buildOpts);
            const packagePath = await (0, project_1.getPackagePath)(this.integration.root, conf.getProjectInfo().name, platform, { emulator: !options['device'], release: !!options['release'] });
            const forwardedPorts = details ? runner.getUsedPorts(runnerOpts, details) : [];
            await this.runNativeRun((0, native_run_1.createNativeRunArgs)({ packagePath, platform, forwardedPorts }, options));
        }
        else {
            await this.runCordova((0, utils_1.filterArgumentsForCordova)(metadata, options), buildOpts);
    ...
    async runBuildDeploy(inputs, options) {
        ...
        if (options['native-run']) {
            const conf = await loadCordovaConfig(this.integration);
            const [platform] = inputs;
            await this.runCordova((0, utils_1.filterArgumentsForCordova)({ ...metadata, name: 'build' }, options), { stdio: 'inherit' });
            const packagePath = await (0, project_1.getPackagePath)(this.integration.root, conf.getProjectInfo().name, platform, { emulator: !options['device'], release: !!options['release'] });
            await this.runNativeRun((0, native_run_1.createNativeRunArgs)({ packagePath, platform }, { ...options, connect: false }));
        }
        else {
    ...
    async checkNativeRun() {
        await (0, native_run_1.checkNativeRun)(this.env);
    }
    async runNativeRun(args) {
        if (!this.project) {
            throw new errors_1.FatalException(`Cannot run ${(0, color_1.input)('ionic cordova run/emulate')} outside a project directory.`);
        }
        await (0, native_run_1.runNativeRun)(this.env, args, { cwd: this.integration.root });
    }
}
