...
exports.CORDOVA_COMPILE_OPTIONS = [
    ...
    },
    {
        ...
        hint: (0, color_1.weak)('[cordova/native-run]'),
    },
    {
        name: 'emulator',
...
exports.CORDOVA_RUN_OPTIONS = [
    ...
    {
        name: 'target',
        summary: `Deploy build to a device (use ${(0, color_1.input)('--list')} to see all)`,
        type: String,
        groups: ["advanced" /* MetadataGroup.ADVANCED */, 'cordova', 'cordova-cli', 'native-run'],
        hint: (0, color_1.weak)('[cordova/native-run]'),
    },
];
...
class CordovaCommand extends command_1.Command {
        ...
        if (!cordova) {
            ...
            if (capacitor && capacitor.enabled) {
                throw new errors_1.FatalException(`Refusing to use Cordova inside a Capacitor project.\n` +
                    `Are you looking for the ${(0, color_1.input)('ionic capacitor')} commands? See ${(0, color_1.input)('ionic capacitor --help')}\n\n` +
                    `If you are switching from Capacitor to Cordova, run: ${(0, color_1.input)('ionic integrations disable capacitor')}\n`);
            }
            ...
            await (0, executor_1.runCommand)(runinfo, ['integrations', 'enable', 'cordova']);
        }
    }
    async preRunChecks(runinfo) {
        ...
        const parts = (0, executor_1.getFullCommandParts)(runinfo.location);
        ...
        if (this.project.directory) {
            ...
            const wwwExists = await (0, utils_fs_1.pathExists)(wwwPath); // TODO: hard-coded
            if (!wwwExists) {
                const tasks = this.createTaskChain();
                tasks.next(`Creating ${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(wwwPath))} directory for you`);
                await (0, utils_fs_1.mkdirp)(wwwPath);
                tasks.end();
            }
        }
        ...
        try {
            ...
            if (e instanceof utils_subprocess_1.SubprocessError) {
                if (e.code === utils_subprocess_1.ERROR_COMMAND_NOT_FOUND) {
                    const installArgs = await (0, npm_1.pkgManagerArgs)(this.env.config.get('npmClient'), { command: 'install', pkg: 'cordova', global: true });
    ...
    async checkForPlatformInstallation(platform, { promptToInstall = !['android', 'ios'].includes(platform), promptToInstallRefusalMsg = `Cannot run this command for the ${(0, color_1.input)(platform)} platform unless it is installed.` } = {}) {
        if (!this.project) {
            throw new errors_1.FatalException('Cannot use Cordova outside a project directory.');
        }
        if (platform) {
            ...
            if (!platforms.includes(platform)) {
                ...
                const confirm = promptToInstall ? await this.env.prompt({
                    message: `Platform ${(0, color_1.input)(platform)} is not installed! Would you like to install it?`,
                    type: 'confirm',
                    name: 'confirm',
                }) : true;
                if (confirm) {
