...
class ProjectDetails {
    ...
    async getIdFromArgs() {
        ...
        if (id) {
            debug(`Project id from args: ${(0, color_1.strong)(id)}`);
            return id;
        }
    }
        ...
        for (const [key, value] of lodash.entries(config.projects)) {
            ...
            if (value && value.root) {
                ...
                if (ctx.execPath.startsWith(projectDir)) {
                    debug(`Project id from path match: ${(0, color_1.strong)(id)}`);
                    return id;
                }
            }
        }
    ...
    async getIdFromDefaultProject(config) {
        ...
        if (id) {
            debug(`Project id from defaultProject: ${(0, color_1.strong)(id)}`);
            return id;
        }
    }
    async getTypeFromConfig(config) {
        const { type } = config;
        if (type) {
            debug(`Project type from config: ${(0, color_1.strong)(prettyProjectName(type))} ${type ? (0, color_1.strong)(`(${type})`) : ''}`);
            return type;
        }
    }
    async getTypeFromDetection() {
        for (const projectType of constants_1.PROJECT_TYPES) {
            ...
            if (await p.detected()) {
                debug(`Project type from detection: ${(0, color_1.strong)(prettyProjectName(type))} ${type ? (0, color_1.strong)(`(${type})`) : ''}`);
                return type;
            }
        }
    }
    async determineSingleApp(config) {
        const errors = [];
        let type = await (0, fn_1.resolveValue)(async () => this.getTypeFromConfig(config), async () => this.getTypeFromDetection());
        if (!type) {
    ...
    async determineMultiApp(config) {
        const errors = [];
        const id = await (0, fn_1.resolveValue)(async () => this.getIdFromArgs(), async () => this.getIdFromPathMatch(config), async () => this.getIdFromDefaultProject(config));
        let type;
        if (id) {
    ...
    processResult(result) {
        ...
        if (e1) {
            log.error(`Error while loading config (project config: ${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(result.configPath))})\n` +
                `${e1.cause ? `${e1.message}: ${(0, color_1.failure)(e1.cause.toString())}` : (0, color_1.failure)(e1.message)}. ` +
                `Run ${(0, color_1.input)('ionic init')} to re-initialize your Ionic project. Without a valid project config, the CLI will not have project context.`);
            log.nl();
        }
        if (result.context === 'multiapp') {
            if (errorCodes.includes('ERR_MULTI_MISSING_ID')) {
                log.warn(`Multi-app workspace detected, but cannot determine which project to use.\n` +
                    `Please set a ${(0, color_1.input)('defaultProject')} in ${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(result.configPath))} or specify the project using the global ${(0, color_1.input)('--project')} option. Read the documentation${(0, color_1.ancillary)('[1]')} for more information.\n\n` +
                    `${(0, color_1.ancillary)('[1]')}: ${(0, color_1.strong)('https://ion.link/multi-app-docs')}`);
                log.nl();
            }
            if (result.id && errorCodes.includes('ERR_MULTI_MISSING_CONFIG')) {
                log.warn(`Multi-app workspace detected, but project was not found in configuration.\n` +
                    `Project ${(0, color_1.input)(result.id)} could not be found in the workspace. Did you add it to ${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(result.configPath))}?`);
            }
        }
        if (errorCodes.includes('ERR_MISSING_PROJECT_TYPE')) {
            const listWrapOptions = { width: utils_terminal_1.TTY_WIDTH - 8 - 3, indentation: 1 };
            log.warn(`Could not determine project type (project config: ${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(result.configPath))}).\n` +
                `- ${(0, utils_terminal_1.wordWrap)(`For ${(0, color_1.strong)(prettyProjectName('angular'))} projects, make sure ${(0, color_1.input)('@ionic/angular')} is listed as a dependency in ${(0, color_1.strong)('package.json')}.`, listWrapOptions)}\n` +
                `Alternatively, set ${(0, color_1.strong)('type')} attribute in ${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(result.configPath))} to one of: ${constants_1.PROJECT_TYPES.map(v => (0, color_1.input)(v)).join(', ')}.\n\n` +
                `If the Ionic CLI does not know what type of project this is, ${(0, color_1.input)('ionic build')}, ${(0, color_1.input)('ionic serve')}, and other commands may not work. You can use the ${(0, color_1.input)('custom')} project type if that's okay.`);
            log.nl();
        }
        if (e2) {
            log.error(`${e2.message} (project config: ${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(result.configPath))}).\n` +
                `Project type must be one of: ${constants_1.PROJECT_TYPES.map(v => (0, color_1.input)(v)).join(', ')}`);
            log.nl();
        }
    }
    async readConfig(p) {
        try {
            let configContents = await (0, utils_fs_1.readFile)(p, { encoding: 'utf8' });
            if (!configContents) {
                configContents = '{}\n';
                await (0, utils_fs_1.writeFile)(p, configContents, { encoding: 'utf8' });
            }
    ...
    async result() {
        ...
        try {
            ...
            if ((0, guards_1.isProjectConfig)(config)) {
                const r = await this.determineSingleApp(config);
                errors.push(...r.errors);
                return { ...r, configPath, errors };
            }
            if ((0, guards_1.isMultiProjectConfig)(config)) {
...
async function createProjectFromDetails(details, deps) {
    ...
    switch (type) {
        case 'angular':
        case constants_1.ANGULAR_STANDALONE:
        ...
        case 'vue-vite':
            const { VueViteProject } = await Promise.resolve().then(() => require('./vue-vite'));
            return new VueViteProject(details, deps);
        case 'react-vite':
            const { ReactViteProject } = await Promise.resolve().then(() => require('./react-vite'));
    ...
    throw new errors_1.FatalException(`Bad project type: ${(0, color_1.strong)(String(type))}`); // TODO?
}
exports.createProjectFromDetails = createProjectFromDetails;
async function findProjectDirectory(cwd) {
    return (0, utils_fs_1.findBaseDirectory)(cwd, constants_1.PROJECT_FILE);
}
...
class Project {
    ...
    async requireAppflowId() {
        ...
        if (!appflowId) {
            throw new errors_1.FatalException(`Your project file (${(0, color_1.strong)((0, utils_terminal_1.prettyPath)(this.filePath))}) does not contain '${(0, color_1.strong)('id')}'. ` +
                `Run ${(0, color_1.input)('ionic link')}.`);
        }
        return appflowId;
    }
    get packageJsonPath() {
    ...
    async getPackageJson(pkgName, { logErrors = true } = {}) {
        ...
        let pkgPath;
        try {
            pkgPath = pkgName ? require.resolve(`${pkgName}/package.json`, { paths: (0, node_1.compileNodeModulesPaths)(this.directory) }) : this.packageJsonPath;
            pkg = await (0, node_1.readPackageJsonFile)(pkgPath);
        }
        catch (e) {
            if (logErrors) {
                this.e.log.warn(`Error loading ${(0, color_1.strong)(pkgName ? pkgName : `project's`)} ${(0, color_1.strong)('package.json')}: ${e}`);
            }
        }
    ...
    async requirePackageJson(pkgName) {
        try {
            const pkgPath = pkgName ? require.resolve(`${pkgName}/package.json`, { paths: (0, node_1.compileNodeModulesPaths)(this.directory) }) : this.packageJsonPath;
            return await (0, node_1.readPackageJsonFile)(pkgPath);
        }
        catch (e) {
            if (e instanceof SyntaxError) {
                throw new errors_1.FatalException(`Could not parse ${(0, color_1.strong)(pkgName ? pkgName : `project's`)} ${(0, color_1.strong)('package.json')}. Is it a valid JSON file?`);
            }
            else if (e === node_1.ERROR_INVALID_PACKAGE_JSON) {
                throw new errors_1.FatalException(`The ${(0, color_1.strong)(pkgName ? pkgName : `project's`)} ${(0, color_1.strong)('package.json')} file seems malformed.`);
            }
    ...
    async getDistDir() {
        if (this.getIntegration('capacitor') !== undefined) {
            ...
            if (webDir) {
                ...
                throw new errors_1.FatalException(`The ${(0, color_1.input)('webDir')} property must be set in the Capacitor configuration file. \n` +
                    `See the Capacitor docs for more information: ${(0, color_1.strong)('https://capacitor.ionicframework.com/docs/basics/configuring-your-app')}`);
            }
        }
        else {
    ...
    async personalize(details) {
        ...
        await (0, utils_fs_1.writeJson)(this.packageJsonPath, pkg, { spaces: 2 });
        if (themeColor) {
    ...
    async writeThemeColor(variablesPath, themeColor) {
        ...
        try {
            let themeVarsContents = await (0, utils_fs_1.readFile)(variablesPath, { encoding: 'utf8' });
            ...
            await (0, utils_fs_1.writeFile)(variablesPath, themeVarsContents);
        }
        catch (e) {
            const { log } = this.e;
    ...
    async setAppResources(appIcon, splash) {
        ...
        try {
            await (0, utils_fs_1.ensureDir)(resourcesDir);
            await (0, utils_fs_1.writeFile)(iconPath, appIcon);
            await (0, utils_fs_1.writeFile)(splashPath, splash);
        }
        catch (e) {
            const { log } = this.e;
    ...
    requireIntegration(name) {
        ...
        if (!integration) {
            throw new errors_1.FatalException(`Could not find ${(0, color_1.strong)(name)} integration in the ${(0, color_1.strong)(id ? id : 'default')} project.`);
        }
        if (!integration.enabled) {
            throw new errors_1.FatalException(`${(0, color_1.strong)(name)} integration is disabled in the ${(0, color_1.strong)(id ? id : 'default')} project.`);
        }
        return integration;
    }
...
function isValidProjectId(projectId) {
    return projectId !== '.' && (0, node_1.isValidPackageName)(projectId) && projectId === path.basename(projectId);
}
