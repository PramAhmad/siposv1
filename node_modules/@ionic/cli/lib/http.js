...
class Client {
    ...
    async make(method, path, contentType = "application/json" /* ContentType.JSON */) {
        const url = path.startsWith('http://') || path.startsWith('https://') ? path : `${this.config.getAPIUrl()}${path}`;
        const { req } = await (0, http_1.createRequest)(method, url, this.config.getHTTPConfig());
        req
            .set('Content-Type', contentType)
            .set('Accept', "application/json" /* ContentType.JSON */);
        return { req };
    }
    async do(req) {
        const res = await req;
        const r = transformAPIResponse(res);
        if ((0, guards_1.isAPIResponseError)(r)) {
...
function transformAPIResponse(r) {
    ...
    if (r.status !== 204 && r.type !== "application/json" /* ContentType.JSON */) {
...
function formatSuperAgentError(e) {
    ...
    return (0, color_1.failure)((0, color_1.strong)(f));
}
exports.formatSuperAgentError = formatSuperAgentError;
function formatAPIResponse(req, r) {
    return formatResponseError(req, r.meta.status, (0, guards_1.isAPIResponseSuccess)(r) ? r.data : r.error);
}
function formatResponseError(req, status, body) {
    return (0, color_1.failure)(`Request: ${req.method} ${req.url}\n` +
