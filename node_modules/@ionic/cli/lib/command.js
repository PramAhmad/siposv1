...
class Command extends cli_framework_1.BaseCommand {
    ...
    createTaskChain() {
        let output;
        const formatter = (0, logger_1.createFormatter)();
        if (this.env.flags.interactive) {
            ...
            this.env.log.handlers = (0, logger_1.createDefaultLoggerHandlers)();
    ...
    async execute(inputs, options, runinfo) {
        if ((0, guards_1.isCommandPreRun)(this)) {
        ...
        try {
            ...
            if (!this.env.flags.interactive) {
                this.env.log.warn(`Command ran non-interactively due to ${(0, color_1.input)('--no-interactive')} flag, CI being detected, non-TTY, or a config setting.`);
            }
            throw e;
        }
        ...
        const telemetryPromise = (async () => {
            if (this.env.config.get('telemetry') !== false && !utils_terminal_1.TERMINAL_INFO.ci && utils_terminal_1.TERMINAL_INFO.tty) {
                ...
                const cmd = this;
                const path = await (0, cli_framework_1.generateCommandPath)(cmd);
    ...
    async getCleanInputsForTelemetry(inputs, options) {
        ...
        const optionInputs = (0, cli_framework_1.unparseArgs)(filteredOptions, { useDoubleQuotes: true });
